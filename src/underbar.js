...
(function() {
  ...
  _.some = function(collection, iterator) {
    // TIP: There's a very clever way to re-use every() here.
		...
		return false;
		_.some with every */
		return !(_.every(collection, function(present) {
  ...
  _.extend = function(obj) {
		...
		for (var i = 1; i < arguments.length; i++) {
			addArg = arguments[i];
			for (var key in addArg) {
				firstArg[key] = addArg[key];
			}
		}
		return firstArg;
  };
  ...
  _.defaults = function(obj) {
		...
		for (var i = 1; i < arguments.length; i++) {
			addArg = arguments[i];
			for (var key in addArg) {
				if (firstArg[key] === undefined) {
					firstArg[key] = firstArg[key] || addArg[key];
				}
			}
		}
		return firstArg;
  ...
  _.intersection = function() {
		...
		for (var i = 0; i < arguments.length; i++) {
			for (var j = 0; j < arguments[i].length; j++) {
				if (present[arguments[i][j]] === true) {
					result.push(arguments[i][j]);
				} else {
					present[arguments[i][j]] = true;
				}
			}
		}
		return result;
